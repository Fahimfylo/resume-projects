  ...
  return (
    <div className="p-6 lg:p-10 pt-20 lg:pt-10">
      <div className="flex items-center justify-between mb-8">
        <div>
          <div className="text-[10px] font-ui text-nexus-jade tracking-[0.5em] mb-2 uppercase">Competition</div>
          <h1 className="text-3xl lg:text-5xl font-headline font-black text-white tracking-tighter uppercase">
            TOURNAMENT <span className="text-nexus-gold">HUB</span>
          </h1>
        </div>
        <Button onClick={() => setShowForm(!showForm)} className="bg-nexus-jade text-nexus-void hover:bg-nexus-jade/80 font-ui text-xs">
      ...
      {showForm && (
        <Card className="glass-panel border-white/10 p-6 rounded-none hud-frame bg-nexus-carbon/40 mb-8">
          ...
          <Button onClick={handleCreate} className="bg-nexus-jade text-nexus-void font-ui text-xs">
          ...
          >
            <div className="flex items-start justify-between gap-4">
              <div>
                ...
                <div className="text-xs font-ui text-white/50">{t.game} • {t.type} • {t.participants?.length || 0}/{t.maxParticipants}</div>
              </div>
              <div className="text-right text-xs font-ui">
                <div className="text-nexus-gold font-headline">${(t.prizePool || 0).toLocaleString()}</div>
