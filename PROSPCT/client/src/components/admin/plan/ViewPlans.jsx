...
import { RiExpandUpDownFill, RiSearchLine, RiAddLine, RiArrowLeftLine, RiPriceTag3Line, RiUserAddLine } from "react-icons/ri";
...
const sortPlansByType = (plansArray) => {
  if (!plansArray || plansArray.length === 0) return [];
  
  // Create a copy to avoid mutating original array
  const plansCopy = [...plansArray];
  
  return plansCopy.sort((a, b) => {
    // Use the name field for sorting
    const nameA = (a.name || "").toLowerCase().trim();
    const nameB = (b.name || "").toLowerCase().trim();
    
    // Match plan names to order keys
...
export default function ViewPlans() {
  ...
  const fetchplanData = useCallback(async () => {
    ...
    try {
      ...
      const sortedPlans = sortPlansByType(plansRes.data.plans || []);
      setPlans(sortedPlans);
      setTotalCount(plansRes.data.totalCount || 0);
      ...
      setError(
        ...
        err?.message ||
        "An error occurred while fetching plan data.",
      );
    } finally {
      setLoading(false);
    }
  ...
  const handleUserSearch = async (query) => {
    ...
    const filtered = users.filter(
      (u) =>
        !(selectedPlan?.assigned || []).includes(u.email) &&
        ...
        <div className="flex gap-2 justify-end mt-5">
          ...
          <button
            onClick={async () => {
              ...
              try {
                const response = await axios.delete(`${BASE_URL}/api/plans/remove-plan-assignment/${userToRemove._id}`, {
                  headers: { Authorization: `Bearer ${token}` },
                });
                ...
                toast.error(err?.response?.data?.message || "Error removing user from plan");
              }
            }}
  ...
  const doAssignUser = async (user) => {
    const token = Cookies.get("adminAccessToken");
    try {
      const response = await axios.post(`${BASE_URL}/api/plans/assign-plan`, {
        userId: user._id,
      ...
      if (response.data.success) {
        ...
        setUserSearchQuery("");
        toast.success("User assigned to plan successfully");
        ...
        setPlans(sortedPlans);
        setTotalCount(plansRes.data.totalCount || 0);
        const refreshed = sortedPlans.find(p => p._id === selectedPlan._id);
        if (refreshed) setSelectedPlan(refreshed);
      } else {
        toast.error(response.data.message || "Failed to assign user");
      }
    } catch (err) {
      toast.error(err?.response?.data?.message || "Error assigning user to plan");
    }
  };
        ...
        <div className="flex gap-2 justify-end mt-5">
          ...
          <button
            onClick={async () => {
              ...
              const token = Cookies.get("adminAccessToken");
              try {
                await axios.delete(`${BASE_URL}/api/plans/delete/${id}`, {
                  headers: { Authorization: `Bearer ${token}` },
                });
                toast.success("Plan deleted successfully!");
                setActiveId(null);
                // Fetch and sort plans after deletion
                const response = await axios.get(`${BASE_URL}/api/plans/`, {
                  headers: { Authorization: `Bearer ${token}` },
                });
                const sortedPlans = sortPlansByType(response.data.plans || []);
                setPlans(sortedPlans);
      ...
      <div className="max-w-7xl mx-auto p-4 md:p-6">
        ...
        {!selectedPlan && plans.length > 0 && (
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
            <div>
              <h1 className="text-2xl font-bold text-gray-800 dark:text-gray-100 tracking-tight">Plan Directory</h1>
              <p className="text-sm text-gray-500 dark:text-gray-400">Overview of all active and inactive subscription plans</p>
            </div>
            ...
            <div className="flex items-center gap-3">
              {totalCount >= 4 ? (
                <button
                  disabled
                  title="A maximum of 4 subscription plans can be active at a time."
                  className="flex items-center gap-2 bg-sky-400 cursor-not-allowed text-white px-4 py-2 rounded-lg text-sm font-medium transition-all shadow-sm opacity-70"
                >
                  <RiAddLine size={18} /> Add Plan
                </button>
              ) : (
                <Link to="/admin/plans/add?type=official" className="flex items-center gap-2 bg-sky-600 hover:bg-sky-700 text-white px-4 py-2 rounded-lg text-sm font-medium transition-all shadow-sm">
                  <RiAddLine size={18} /> Add Plan
                </Link>
              )}
            </div>
        ...
        {!selectedPlan ? (
          plans.length > 0 ? (
                        ...
                        <td className="px-6 py-4 text-right relative">
                          ...
                          {activeId === plan._id && (
                            <div ref={dropdownRef} className="absolute right-6 top-10 w-32 bg-white dark:bg-gray-800 border dark:border-gray-700 shadow-xl rounded-lg z-30 py-1">
                              <button onClick={(e) => { e.stopPropagation(); navigate(`/admin/update-plan/${plan._id}`, { state: { plan } }); }} className="w-full text-left px-4 py-2 text-sm text-gray-700 dark:text-gray-300 hover:bg-sky-50 dark:hover:bg-sky-900/30">Update</button>
            ...
            <EmptyState
              ...
              subtitle="Create your first subscription plan to get started. You can add up to 4 plans."
              actionLabel="Add Plan"
              actionPath="/admin/plans/add"
              size="lg"
            ...
            <div className="p-8 grid md:grid-cols-3 gap-8">
              ...
              <div className="md:col-span-2 space-y-8">
                <div className="grid grid-cols-2 md:grid-cols-3 gap-6">
                  ...
                  <div><label className="text-[10px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-widest block mb-1">Seats</label><p className="text-lg font-bold text-gray-900 dark:text-gray-100">{selectedPlan.maxUsers || 1}</p></div>
                </div>
                ...
                <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-6">
                  ...
                  {searchResults.length === 0 && userSearchQuery.trim() && !searchingUsers && (
                ...
                <div className="flex gap-4">
                  <button onClick={(e) => { e.stopPropagation(); navigate(`/admin/update-plan/${selectedPlan._id}`, { state: { plan: selectedPlan } }); }} className="flex-1 bg-sky-600 text-white py-2.5 rounded-lg font-semibold hover:bg-sky-700 transition-shadow shadow-md">Update Plan Details</button>
