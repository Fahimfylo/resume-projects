...
export default function UpdatePlan() {
    ...
    const fetchPlanData = async () => {
      ...
      try {
        const [planRes, usersRes] = await Promise.all([
          axios.get(`${BASE_URL}/api/plans/${planId}`, {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.put(`${BASE_URL}/api/plans/${planId}`, validatedData, {
        headers: { Authorization: `Bearer ${token}` },
      });
          ...
          <button
            onClick={async () => {
              ...
              try {
                ...
                toast.error(error?.response?.data?.message || "Error removing user from plan");
              }
            }}
  ...
  const doAssignUser = async (user) => {
    ...
    try {
      const response = await axios.post(
        `${BASE_URL}/api/plans/assign-plan`,
        { userId: user._id, planId: planId },
        { headers: { Authorization: `Bearer ${token}` } }
      );
      ...
      if (response.data.success) {
        ...
        const planRes = await axios.get(`${BASE_URL}/api/plans/${planId}`, {
      ...
      toast.error(error?.response?.data?.message || "Error assigning user to plan");
    }
  };
      ...
      <AdminComponent>
        <div className="fixed inset-0 flex items-center justify-center bg-white/80 dark:bg-gray-900/80 backdrop-blur-sm">
          <div className="text-center">
            ...
            <p className="text-gray-600 dark:text-gray-300 font-medium">Loading plan data...</p>
          </div>
        </div>
      </AdminComponent>
    );
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <h1 className="text-2xl font-bold text-gray-800 dark:text-gray-100 tracking-tight flex items-center gap-2">
              <RiPriceTag3Line className="text-sky-500 dark:text-sky-400" /> Update Plan Detail
            </h1>
          </div>
        </div>
